...
function StudentActivity() {
    ...
    client.get('/api/qr-session', { params: { action: 'lookup', code: sessionCode } })
      .then(res => {
        ...
        if (res.data?.data?.activity_type) {
          ...
          if (found) { setSelectedType(found); setView('write'); }
        }
      })
  ...
  const handleSave = async (isDraft = false) => {
    ...
    if (!isDraft) {
      setTimeout(() => { setSelectedType(null); setTitle(''); setContent(''); setSaveStatus(''); setView('select'); }, 2000);
    }
  };
  ...
  if (view === 'manuscript') {
    return (
      <div className="space-y-3">
        <button onClick={() => setView('select')} className="text-xs text-gray-400 hover:text-gray-600">← 활동 선택</button>
        <Suspense fallback={<div className="text-center py-8 text-gray-400 text-xs">로딩 중...</div>}>
          <ManuscriptActivity embedded />
        </Suspense>
      </div>
    );
  }

  if (view === 'write' && selectedType) {
    return (
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { setSelectedType(null); setTitle(''); setContent(''); setSaveStatus(''); setView('select'); }}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {ACTIVITY_TYPES.map(type => (
          <button key={type.id} onClick={() => { setSelectedType(type); setView('write'); }}
