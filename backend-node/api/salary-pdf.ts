...
// PDF 텍스트는 프론트엔드(브라우저)에서 pdfjs-dist 로 추출해서 전송.
// 서버에서 pdf-parse 를 쓰면 @napi-rs/canvas 의존으로 Vercel 서버리스 크래시.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const { userId, pdfText } = body || {};

    if (!userId) return res.status(400).json({ message: 'userId가 필요합니다.' });
    if (!pdfText || typeof pdfText !== 'string' || pdfText.length < 50) {
      return res.status(400).json({ message: 'PDF 텍스트가 비어 있습니다.' });
    }

    const parsed = parseSalaryText(pdfText);
    const db = getPool();
